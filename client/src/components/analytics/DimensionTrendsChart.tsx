...
// Dimension colors - matching the standard palette
const DIMENSION_COLORS = {
  leadershipEffectiveness: "#3b82f6", // Blue
  teamWellbeing: "#22c55e", // Green
  burnoutRisk: "#ef4444", // Red (inverted - higher is worse)
  psychologicalSafety: "#a855f7", // Purple
...
export const DimensionTrendsChart: React.FC<DimensionTrendsChartProps> = ({
  ...
  if (!data || data.trends.length === 0) {
    return (
      ...
      >
        <div className="flex flex-col items-center justify-center h-64 text-center">
          <TrendingUp className="w-10 h-10 mb-3 text-gray-400" />
          <p className="text-lg font-semibold text-gray-700">No Trend Data</p>
  ...
  if (!data.hasMultipleVersions) {
    ...
    return (
      ...
      >
        <div className="flex flex-col items-center justify-center h-64 text-center">
          <Info className="w-10 h-10 mb-3 text-blue-500" />
          <p className="text-lg font-semibold text-gray-700">Single Version Available</p>
          <p className="text-sm text-gray-500 mt-1">
            Create additional scoring versions to see trends over time.
          </p>
          {avgScore !== null && (
            <div className="mt-4 p-4 bg-blue-50 rounded-lg">
              <p className="text-sm text-blue-700">Current Average Score</p>
              <p className="text-3xl font-bold text-blue-900">{avgScore.toFixed(1)}</p>
            </div>
          )}
        </div>
      </AnalyticsSectionShell>
