...
export default function Header({ showActions = true }: HeaderProps) {
  ...
  return (
    <header className="sticky top-0 z-50 w-full backdrop-blur supports-[backdrop-filter]:bg-[var(--color-surface)]/60 flex h-14 items-center justify-end pr-6 gap-3" style={{ backgroundColor: 'var(--color-surface)', borderBottomColor: 'var(--color-border)', borderBottomWidth: '1px' }}>
      {typedUser && (
        <>
          <Button
            variant="ghost"
