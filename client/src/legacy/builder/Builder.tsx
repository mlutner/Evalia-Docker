/**
 * LEGACY WIZARD BUILDER
 * Deprecated. Do not use for new features.
 * 
 * This was the original 4-step wizard builder that has been replaced by
 * SurveyBuilderV2 (3-panel layout). Kept for reference only.
 * 
...
export default function Builder() {
  ...
  const handleFileSelect = async (file: File) => {
    fileProcessing.handleFileSelect(
      ...
      () => {
        // Navigate to Step 3 (QuestionsStep) - not Step 2 which is still SurveyStartFlow
  ...
  const handleSendMessage = async (message: string, fileData?: { name: string; type: string; base64: string }) => {
    ...
    if (result && result.questions) {
      surveyState.setCurrentQuestions(result.questions);
      // If questions were generated while on start flow, navigate to Questions step
      if ((surveyState.currentWizardStep === 1 || surveyState.currentWizardStep === 2) && result.questions.length > 0) {
        setTimeout(() => surveyState.setCurrentWizardStep(3), 300);
      }
    }
  };
