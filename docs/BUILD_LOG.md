...
- 2025-12-13: **[DEV-INFRA-001] Vite Proxy + Engine Key Fix** – Fixed two infrastructure issues causing "no templates/surveys" display bug. (1) Added `/api` proxy to `vite.config.ts` so the app works correctly when accessed on Vite's default port (`:5173`) instead of the Express server port (`:4000`). Previously, API calls from `:5173` would 404/CORS fail silently. (2) Fixed duplicate object keys in `client/src/core/logic/engines.ts` where `id: "LogicV3"` and `name: "Logic V3"` were overwriting `id: "logicEngineV3"` and `name: "Logic Engine V3"`. This caused potential engine ID mismatch issues. Both fixes committed as `4f479b9`.

- 2025-12-13: **[ANAL-OUT-005] Widget State System for Analytics Widgets** – Implemented unified widget state system to ensure no widget is ever blank without an explanation. Created `client/src/utils/widgetState.ts` with `getWidgetState()` helper that returns state classification (`ready`|`loading`|`insufficient_data`|`no_signal`|`version_mismatch`|`error`) with human-readable title, message, and severity. Created `client/src/components/analytics/WidgetStateDisplay.tsx` with: (1) `WidgetStateDisplay` - renders appropriate UI for widget states with test IDs, (2) `WidgetLoadingSkeleton` - loading skeletons for different widget types, (3) `WidgetVersionWarning` - inline warning banner for version mismatch, (4) `WidgetStateWrapper` - convenience wrapper for automatic state handling. Updated four key widgets to use the system: **HotspotList** (added `InsufficientDataState`, `NoSignalState`, data-testid attributes), **ActionQueueList** (added `CriticalOutcomesNoActionsState` explaining why critical outcomes exist but no action items), **IndexDistributionChart** (added `InsufficientDataState` for <10 responses), **DimensionTrendsChart** (added `InsufficientDataState` for first measurement period). All widgets now show version mismatch warnings when using fallback version. Added data-testid attributes for Playwright E2E testing: `*-widget`, `*-widget-state-loading`, `*-widget-state-error`, `*-widget-state-insufficient`, `*-widget-state-no-signal`, `*-widget-content`. TypeScript types pass. This is partial implementation of ANAL-OUT-005; report-first layout refactor and E2E tests pending.

- 2025-12-12: **[EXEC-BRIEF-001] Executive Brief Dashboard View** – Added new "Executive Brief" tab to Analytics page for 5D surveys. Created `client/src/components/analytics/ExecutiveBriefView.tsx` as a single-file MVP component adapted from Magic Patterns prototype. Features: (1) Overall Health Score hero with circular gauge and health status badge, (2) Dimension outcome cards with polarity-consistent displayScore/displayLabel (burnout shows as "Burnout Resilience"), (3) Action Queue section with severity filtering and search, (4) Hotspots ranked list showing segment-level issues, (5) Collapsible Data Quality & Methodology section, (6) Detail drawer with dimension-specific playbooks. Data is wired to existing hooks (outcomeSummaryData, actionQueueResult, hotspotData, participationMetrics) with adapter functions (`toExecutiveOutcome`, `toExecutiveActionItem`, `toExecutiveHotspot`, `buildSurveyMeta`). Tab only visible for 5D dashboards. Uses version-locked data via selectedVersion prop. Build passes with no unused imports.

- 2025-12-12: **[VERSION-TRUTH-001] Version Mismatch Warning Banner** – Added prominent warning banner to AnalyticsPage when the selected version doesn't exist and falls back to latest. Uses `versionConsistency.warning` from `getVersionConsistency()` helper. Banner shows "Version Not Found" with detailed message explaining which version was requested and which is being displayed instead. Helps debug regressions where modules might return different versions than expected.

- 2025-12-12: **[EXPORT-ALIGN-001] Export Polarity Alignment with UI** – Updated diagnostic report export to use displayScore/displayLabel for polarity-consistent display. Added `displayLabel`, `displayShortLabel`, `displayScore`, `displayDelta` fields to `DiagnosticOutcome` interface in `server/utils/diagnosticReportExport.ts`. Added `POSITIVE_FRAMING_LABELS` constant for burnout-risk → "Burnout Resilience" mapping. Updated `server/utils/diagnosticReportPdf.ts` to use `displayShortLabel` in table column and `displayScore` for score values. Removed "(Lower scores are better)" note from dimension drivers section since displayScore is always "higher = better". Export now matches UI framing for reversed dimensions.

- 2025-12-12: **[DASH-IA-001] Insights Home Action Queue Layout** – Replaced duplicate KeyInsights/FocusAreas/Hotspots widgets on Insights Home with single unified ActionQueueList component. Created `client/src/components/analytics/ActionQueueList.tsx` showing ranked action items from `buildActionQueue()` adapter. Items sorted by severity (critical first) then score (lowest first). Shows severity summary badges (N critical, N need attention), suppressed items note when data insufficient. Empty state shows "All dimensions performing well" message. Removed separate HotspotList, KeyInsightsSummaryCard, FocusAreasCard from Insights Home. Kept OutcomeSummarySection at top. 11 new tests for ActionQueueList component.

- 2025-12-12: **[TEST-001] ScoringView Test Selector Stabilization** – Fixed brittle text-based selectors in ScoringView.test.tsx by adding data-testid attributes to ScoringNavigator.tsx. Updated test assertions to use `getByTestId("categories-section")`, `getByTestId("bands-section")`, `getByTestId("category-item-{id}")`, `getByTestId("band-item-{id}")` instead of text-based selectors like `getByText("Categories")`. This prevents test failures when UI copy changes. All 2 tests pass.

- 2025-12-12: **[DASH-IA-001-PREP] Action Queue Data Adapter** – Created `client/src/utils/actionQueue.ts` with comprehensive Action Queue adapter for the upcoming Intelligent Actions dashboard. Computes a unified ranked list from outcomes, hotspots, and domain overview data. Key features: (1) severity-first ranking (critical → warning → neutral → good), (2) score-based secondary sort (lowest first for attention), (3) data sufficiency gating (hotspots suppressed when responseCount < MIN_HOTSPOT_RESPONSES), (4) deduplication between org-level and segment-level items. Interfaces: `ActionQueueItem`, `ActionDriver`, `ActionCtas`, `ActionQueueOptions`. Main function: `buildActionQueue(outcomes, hotspots, domainOverview, options)`. 37 unit tests covering ranking, gating, deduplication, and integration scenarios.

- 2025-12-12: **[ANAL-VERSION-001] Version-Locked Analytics Display** – Implemented version-locking across all analytics widgets to ensure all modules use the same selected versionId, eliminating mixed-version rendering. Created `client/src/utils/versionedAnalytics.ts` with: (1) `findTrendByVersion()` - finds trend point for specific version with latest fallback, (2) `findPreviousTrend()` - finds previous trend for delta calculations, (3) `getVersionConsistency()` - provides version mismatch detection with warnings. Updated `AnalyticsPage.tsx` to derive currentTrend and previousTrend from selectedVersion instead of always using latest. Added versionId parameter to hotspot hook. Added version indicator badge in header showing resolved version label. 23 unit tests covering version selection, fallback behavior, and consistency checks.

- 2025-12-12: **[UI-OUT-003] Polarity-Consistent Outcome Display + Data Sufficiency Gating** – Implemented "health score framing" where all dimension scores display with higher = better semantic. **Part A:** For reversed dimensions (burnout-risk), OutcomeCard now shows `displayScore` (inverted) as the big number with positive-framed label "Burnout Resilience" instead of "Burnout Risk". Removed confusing "↓ Lower is better" badge and separate "Performance score: X" line. Added `displayLabel`, `displayShortLabel`, `displayScore`, `displayDelta` fields to `OutcomeData` interface. Updated `ResultsScreen` strengths/focus to use `displayLabel` and `displayPct`. **Part B:** Created `client/src/utils/dataSufficiency.ts` with `getDataSufficiency()` helper returning `canShowTrend`, `canShowDistribution`, `canShowHotspots` flags based on thresholds (10+ responses for distribution, 5+ per segment for hotspots, 2+ periods for trend). Trend label now returns "Baseline" instead of misleading "Stable" when no previous comparison period. Added `BaselineIndicator` component to `OutcomeSummarySection`. 53 new unit tests covering threshold behavior and polarity display.

- 2025-12-12: **[ANAL-OUT-002] Global ScoreRanges Fallback in calculateSurveyScores** – Fixed bug where 5D dimension scores returned all zeros for surveys using global scoreRanges (bands without per-category `rule.category` field). Root cause: `calculateSurveyScores()` filtered bands by `rule.category === catId`, returning empty array for global bands. Added `getBandsForCategory()` helper in `shared/schema.ts` that checks for category-specific bands first, then falls back to global bands (where `!rule.category`). Applied fallback in both normalization phase (score calculation) and interpretation phase (band label assignment). API now returns real dimension scores (57, 70, 83, 53, 53 vs all 0s). 7 unit tests passing. Debug logging removed per ticket requirement.

- 2025-12-12: **[TEST-001] Scoring Builder Tests Restored** – Fixed 5 failing tests in the scoring builder test suite. Updated `ScoringView.test.tsx` to match current 3-panel layout (tests were written for old tab-based UI). Updated `AnalyticsPage.integration.test.tsx` to use CSS variable selectors (`[class*="status-error"]`) instead of hardcoded color names (`[class*="red"]`). Fixed `builderLifecycle.test.tsx` and `store.operations.test.tsx` to skip validation when testing HTTP methods (empty surveys fail validation). Fixed `responses.scoring.test.ts` to handle 4th argument (versionId) in `createResponse` call signature. All 520 tests now pass.

- 2025-12-12: **[RES-001B] ResultsScreen Source-of-Truth Enforcement** – De-duplicated ranking logic into `client/src/utils/resultsRanking.ts` with `computeEffectivePct`, `selectStrengthsAndFocus`, `buildCategoryResult`, and `CategoryResult` interface. Added `ResultsMode` type (`'preview' | 'respondent'`) to enforce server scoring in respondent flow. Preview mode allows local scoring fallback for builder. Respondent mode shows "Results Unavailable" if server scoring missing. Updated `SurveyView.tsx` to pass `mode="respondent"`. Added deterministic tie-breaker sorting (effectivePct desc, then label asc). 19 unit tests passing.

- 2025-12-12: **[QA-EXPORT-001] Export Parity + Determinism Regression Checks** – Added comprehensive test suite validating export data matches diagnostic computations and is deterministic across runs. Created `server/__tests__/export/exportParity.test.ts` with 36 tests covering: outcome computation parity (healthy org, burnout hotspot, low psych safety fixtures), hotspot transformation, full export assembly, and determinism assertions. Tests verify: same activeDimensions + isReversed flags, same warnings, stable dimension/hotspot/driver ordering, same severity/trend labels, and stable methodology notes. The `generatedAt` field is explicitly excluded from determinism guarantees (runtime timestamp). Burnout-risk reversed scoring is correctly handled in simulated score computation. Updated `docs/templates/fixtures.md` with "Export Expectations" section documenting determinism guarantees and non-deterministic fields.

- 2025-12-12: **[TMP-001] 5D Long-Form + Segmentation-Ready Templates** – Created diagnostic-grade 5D organizational health templates with segmentation support. Created `server/templates/evalia5DCoreTemplate.ts` with 30 scored questions (6 per dimension), 5 segmentation fields (department, team, managerId, role, location), and 2 open-ended questions. Created `server/templates/evalia5DLongTemplate.ts` with 50 scored questions (10 per dimension: 6 Core + 4 deep-dive), same segmentation, and 3 open-ended questions. Deep-dive modules add granularity for recognition, career growth, communication, feedback culture, workload, recovery capacity, etc. All categories have explicit `dimensionId` mappings (no legacy fallback needed). Created `server/templates/fixtures/analyticsFixtures.ts` with 3 deterministic test fixtures: "Healthy Org" (40 respondents, all dimensions 70-85), "Burnout Hotspot Department" (32 respondents, IT dept elevated burnout), "Low Psych Safety Manager" (36 respondents, MGR-001 critical psych safety). 53 new unit tests validating template structure, question counts, polarity, fixture data quality. Created docs: `docs/templates/5d-core.md`, `docs/templates/5d-long.md`, `docs/templates/fixtures.md`.

- 2025-12-12: **[EXPORT-001] Diagnostic Report PDF Export** – Implemented deterministic, lens-aware PDF export of Evalia's diagnostic outputs. Created `server/utils/diagnosticReportExport.ts` with `assembleDiagnosticReportData()` to collect Outcome Summary, Hotspots, Domain Overview, and metadata (activeDimensions, warnings, diagnosticLensId). Created `server/utils/diagnosticReportPdf.ts` using jsPDF for server-side PDF generation with 5-section structure: Cover Page, Executive Summary (dimension table with scores/bands/trends), Key Hotspots (ranked by priority), Drivers by Dimension (top 3 worst categories), and Data Quality & Methodology (warnings, confidence level, limitations). Added `GET /api/analytics/:surveyId/export/pdf` and `GET /api/analytics/:surveyId/export/data` endpoints in `server/routes/analytics.ts`. Added "Export PDF" button to AnalyticsPage header (only for scored surveys). Respects reversed dimensions (burnout-risk), includes all warnings explicitly, generates methodology notes based on data quality. 19 new unit tests validating payload includes diagnosticLensId, activeDimensions, warnings. No AI-generated content.

- 2025-12-12: **[HOT-001] Hotspots v1 – Real implementation replacing stub** – Implemented `computeHotspotSummary()` in `server/utils/analytics.ts` to replace stub endpoint. Computes hotspots from actual response data grouped by segment (manager/team). A hotspot is a segment with at least one dimension in Critical (score ≤39 or burnout ≥61) or Warning (score 40-54 or burnout 46-60) severity. Hotspots ranked by: 1) priority (high/medium/low based on critical/high issue counts), 2) number of critical issues, 3) total issues. Created `HotspotList` frontend component with `HotspotCard` showing severity badges, issue counts, and priority indicator. Integrated into AnalyticsPage below OutcomeSummarySection. Added `useHotspotSummary` hook. 27 new backend tests for issue detection and priority assignment.

- 2025-12-12: **[OUT-001] Outcome Summary v1** – Added outcome-first analytics section at top of AnalyticsPage. Created `OutcomeCard` and `OutcomeSummarySection` components in `client/src/components/analytics/`. Added `computeOutcomeSummary()` and helpers in `client/src/utils/outcomeComputation.ts`. Shows 3–5 dimension outcome cards sorted by severity (worst first). Respects `meta.activeDimensions[].isReversed` for correct trend interpretation (no hardcoded burnout logic). Shows warning when metadata missing. 25 new tests for outcome computation.

- 2025-12-12: **[DIM-001-E] Frontend consumption of dimension metadata** – Added `useDimensionMeta` hook and `createDimensionMetaHelpers()` in `client/src/hooks/useDimensionMeta.ts` to parse `meta.activeDimensions` and `meta.warnings` from analytics API responses. Updated `AnalyticsPage.tsx` to derive `isReversed` from API meta instead of hardcoding (removed burnout-specific logic). Created `DimensionMappingWarningBanner` component to display unmapped category warnings. 13 new tests for dimension meta helpers. Fallback to hardcoded burnout-risk check maintained for backwards compatibility.

- 2025-12-12: **[DIM-001-D] Dimension metadata in analytics API responses** – Added `computeDimensionMetadata()` helper to `server/routes/analytics.ts`. Extended `AnalyticsMeta` interface with `activeDimensions?: ActiveDimensionInfo[]` and `warnings?: DimensionMappingWarning[]`. Updated 5 endpoint groups to include dimension metadata: `INDEX_TRENDS_SUMMARY`, `BEFORE_AFTER_INDEX_COMPARISON`, Index Distribution (5), Index Band Distribution (5), and Index Trend (3). Frontend now receives `isReversed` flag from server for burnout dimension. 11 new tests for response metadata structure.

- 2025-12-12: **[DIM-001-C] Dimension mapping functions with explicit→legacy→null resolution** – Added `resolveCategoryDimension()`, `collectMappingWarnings()`, and `getActiveDimensions()` to `server/utils/analytics.ts`. Updated `computeTrendPointForVersion` and `calculateVersionScores` to use new mapping. Renamed `CATEGORY_TO_DIMENSION_MAP` to `LEGACY_CATEGORY_TO_DIMENSION_MAP` with deprecation notice. Added `DimensionMappingWarning`, `ActiveDimensionInfo` types and `isDimensionReversed()`, `getActiveDimensionInfo()` helpers to `shared/analytics.ts`. 17 new unit tests. Unmapped categories are now explicitly excluded from 5D aggregation (no silent nulls).

- 2025-12-12: **[DIM-001-A] Added dimensionId to scoreConfig.categories schema** – Extended category schema with optional `dimensionId: InsightDimensionId` field for explicit category→dimension mapping. Added `insightDimensionIds` array, `insightDimensionIdSchema`, and `ScoreConfigCategory` type. 15 new validation tests. Backwards compatible (existing surveys without dimensionId still validate).
